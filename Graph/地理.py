...
# xlsx路径
path = '/Users/fuuujy/Desktop/三常知识图谱/地理三元组.xlsx'
data = xlrd.open_workbook(path)  # 打开Excel文件读取数据
table = data.sheets()[0]  # 通过索引顺序获取
# xlsx循环
for j in range(1, table.nrows):  # 循环行列表数据
    # 一级标题
    ...
    print("title3=", title3)
    node_a = matcher.match(title3, name=title3, source1='地理', source2=title1, source3=title2).first()
    if not node_a:
        node_a = Node(title3, name=title3, source1='地理', source2=title1, source3=title2)
        graph.create(node_a)
        print(node_a)
    # 关系
    ...
    print("title5=", title5)
    node_c = matcher.match(title5, name=title5, source1='地理', source2=title1, source3=title2).first()
    if not node_c:
        node_c = Node(title5, name=title5, source1='地理', source2=title1, source3=title2)
        graph.create(node_c)
        print(node_c)
    # 节点1-（关系）->节点2
